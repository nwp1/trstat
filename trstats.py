    ...
    for lines in lines:
        ...
        for item in contents:
            if( ((item.count(".") > 1 and item != "ms") ) or "gateway" in item):
                temphold.append(item)
    
    i = 0
    for i in range(len(temphold) - 1):
        if diction.get(hop) is None:
            diction[hop] = [(temphold[i],temphold[i+1])]

        else:
            diction[hop].append(temphold[i],temphold[i+1])
...
def tracerouterun( args ):
    ...
    tracerun = f"for i in `seq 1 {args.NUM_RUNS if args.NUM_RUNS is not None else 1}` ; do "+ tracerun + " > test_files/tr_run-$i.txt ; " 
